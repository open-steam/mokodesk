...
class doc_content
{
  ...
  **/
  function _get_textplain($config_webserver_ip, $content)
  {
    ...
    $content = preg_replace('/\[\[Bild:http:([^\]\]]*)\|(.*)\]\]/U', '<div class="wikiimage"><img src="http:\1" alt="\2" title="\2"></div>', $content);
    $content = preg_replace('/\[\[Bild:([^\]\]]*)\|(.*)\]\]/U', '<div class="wikiimage"><img src="' . $config_webserver_ip . '/tools/get.php?object=\1" alt="\2" title="\2"></div>', $content);
...
//      $content = utf8_encode($content);
  ...
  /**
  * Get text content with all links in src="..." and href="..." set to the adequat link according to the bid-owl system
  * @access   private
  **/
  function _get_texthtml($config_webserver_ip, $content)
  {
    ...
    while( $tag_open = strpos($html, '<') !== false )
    {
      ...
      if(eregi("(^link href|^href|^src|^action|^data|^code)*(link href|href|src|action|data|code)=[\"\']([^\"^\']*)", $entity, $regs))
      {
        ...
        if(!isset($split["scheme"]) && isset($split["path"]))
        {
          ...
          $transaction[$transaction_key] = steam_factory::path_to_object( $this->steam, $path );
